...
def build(notes: list, append: bool = False):
    ...
    # 用 分类 + 标题 + 正文 作为 embedding 输入
    texts = [f"[{n.get('category','')}] {n['title']}。{n['content']}" for n in notes]
...
def main():
    ...
    if args.seed:
        ...
        if os.path.exists(config.seed_file):
            ...
            tagged = sum(1 for n in notes if n.get("style_tags"))
            print(f"[build_kb] 从 seed_notes.json 读取了 {len(notes)} 篇种子笔记"
                  f"（其中 {tagged} 篇包含 style_tags 风格标记）")
        else:
            print(f"[build_kb] seed_notes.json 不存在，跳过")
